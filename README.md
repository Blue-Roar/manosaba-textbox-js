...
6. ollama本地模型配置教程：
   ...
   - 在设置内选择使用ollama，模型名称填写为`hf.co/OmniDimen/OmniDimen-4B-Emotion-GGUF-q4_K_M:Q4_K_M`（觉得名字太长可以用ollama cp指令复制一份，不会占用额外硬盘空间），然后点击测试链接，或在主界面点击情感匹配并等待连接即可
...
4. 添加更多表情,并校准
5. 压缩图片
6. 调整部分逻辑，比如启用情感匹配后预览不该随机刷新？ (v1.5)
7. 调整情感列表(感觉目前的分类不是很好用) (v1.5)
8. 程序界面外观优化? (v1.6)
9. 支持编辑人物名字的文字样式以及微调人物位置(v1.8)
