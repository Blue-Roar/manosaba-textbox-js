...
3. 背景不限制文件名
4. 关闭样式窗口时预览多次刷新
5. 支持使用psd作为角色素材（用图层组区分不同姿势，在内部再使用图层组区分表情以及动作）
6. 情感匹配连接测试失败后需要显示错误消息
7. 生成预览时传递的背景和角色索引参数需要移除
8. 程序中其它针对单一角色的索引的修改需要修改
9. 组件逻辑优化：
   + 使用固定角色或背景时，需要锁定主页的相应部分？
   + 样式窗口开启状态下，允许移动主窗口
   + 样式界面按钮没有汉化

## BUG记录
1. 强调字符只有半边时着色有问题（暂时没想着修）
2. 复杂的组合emoji无法正常显示，比如 👨‍👩‍👧‍👦 👨🏻‍👩🏼‍👧🏽‍👦🏾 （不修）
3. 快捷键编辑页无法用滚轮滚动
